...
def get_rects(img_):
    ...
    # cv2.imwrite(r"d:\fp7.png", img)
    # img = morphological_(img)
    # cv2.imwrite(r"d:\fp8.png", img)
...
def sobel_(img):
    # cv2.imwrite(r"d:\fp2.png", img)
    img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    # img = cv2.equalizeHist(img)
    # cv2.imwrite(r"d:\fp3.png", img)
    # 高斯平滑
    # img = cv2.GaussianBlur(img, (3, 3), 0, 0, cv2.BORDER_DEFAULT)
    # img = cv2.equalizeHist(img)
    # cv2.imwrite(r"d:\fp4.png", img)
    # 中值滤波
    # median = cv2.medianBlur(img, 0)
    # cv2.imwrite(r"d:\fp5.png", median)
    median = img
    # Sobel算子，X方向求梯度
    sobel = cv2.Sobel(median, cv2.CV_8U, 1, 0, ksize=3)
    # 二值化
    ret, binary = cv2.threshold(img, 100, 255, cv2.THRESH_BINARY)
    # cv2.imwrite(r"d:\fp6.png", binary)
    return binary
...
def color_(img, flag):
    ...
    UpperBlue = [np.array([180, 255, 180]), np.array([10, 255, 255]), np.array([180, 255, 255]),
                 np.array([124, 255, 255])]
    if flag == 0:
        mask_ = cv2.inRange(HSV.copy(), LowerBlue[3], UpperBlue[3])
    if flag == 1:
        mask_ = cv2.inRange(HSV.copy(), LowerBlue[1], UpperBlue[1]) + cv2.inRange(HSV.copy(), LowerBlue[2],
                                                                                  UpperBlue[2])
    if flag == 2:
...
def find_region(img):
    ...
    for rect in rect_list:
        ...
        if ratio > 20 or ratio < 3:
            continue

        copy = img.copy()
        w1, w2 = rect[0], rect[0] + rect[2]
        h1, h2 = rect[1], rect[1] + rect[3]
        box = [[w1, h2], [w1, h1], [w2, h1], [w2, h2]]
        cv2.drawContours(copy, np.array([box]), 0, (0, 255, 255), 10)
        ...
        # 判断高度和宽带是否满足要求
        if width < w_img / 10 or width > w_img / 1.4 or height < h_img / 60 or height > h_img / 15:
            continue
        ...
        if rect[0] > w_img / 2 and rect[1] < h_img / 2:
            region.append(rect)
        # 含税总价在中下（四分之一）部
        elif rect[0] < w_img / 3 and rect[1] > h_img / 2:
            region.append(rect)
    # return rect_list
    return region
